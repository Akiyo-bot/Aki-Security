...
module.exports.run = async (client, message) => {
    ...
    message.channel.awaitMessages(filter, { max: 1, time: 30000 })
        .then(async (collectedTitle) => {
            if(!collectedTitle.first().content) return message.channel.send('<:false:881803557274255381> Merci de spécifier un texte valide!');

            await title.delete().catch(() => {});

            if(collectedTitle.first().content.toLowerCase() !== 'skip') {
                if(collectedTitle.first().content.length > 256) return message.channel.send('<:false:881803557274255381> Votre titre est trop long.');
            ...
            message.channel.awaitMessages(filter, { max: 1, time: 20000 })
                .then(async (collectedColor) => {
                    if(!collectedColor.first().content) return message.channel.send('<:false:881803557274255381> Merci de spécifier une couleur!');
                    ...
                    if(collectedColor.first().content.toLowerCase() !== 'skip') {
                        ...
                        if(!isValid) return message.channel.send('<:false:881803557274255381> Couleur invalide');
                    ...
                    message.channel.awaitMessages(filter, { max: 1, time: 50000 })
                        .then(async (collectedDesc) => {
                            if(!collectedDesc.first().content) return message.channel.send('<:false:881803557274255381> Merci de spécifier du texte valide!');

                            await description.delete().catch(() => {});

                            if(collectedDesc.first().content.toLowerCase() !== 'skip') {
                                if(collectedDesc.first().content.length > 2048) return message.channel.send('<:false:881803557274255381> Votre description est trop longue.');
                            ...
                            message.channel.awaitMessages(filter, { max: 1, time: 30000 })
                                .then(async (collectedThumbnail) => {
                                    if(!collectedThumbnail.first().content && !collectedThumbnail.first().attachments.first()) return message.channel.send('<:false:881803557274255381> Merci d\'envoyer une image ou un lien.');
                                    ...
                                    if(collectedThumbnail.first().content.toLowerCase() !== 'skip') {
                                        if(collectedThumbnail.first().attachments.first()) {
                                            ...
                                            if(!collectedThumbnail.first().content.startsWith('https://')) return message.channel.send('<:false:881803557274255381> Merci d\'envoyer un lien valide!');
                                    ...
                                    message.channel.awaitMessages(filter, { max: 1, time: 30000 })
                                        .then(async (collectedImage) => {
                                            if(!collectedImage.first().content && !collectedImage.first().attachments.first()) return message.channel.send('<:false:881803557274255381> Merci d\'envoyer une image ou un lien.');
                                            ...
                                            if(collectedImage.first().content.toLowerCase() !== 'skip') {
                                                if(collectedImage.first().attachments.first()) {
                                                    ...
                                                    if(!collectedImage.first().content.startsWith('https://')) return message.channel.send('<:false:881803557274255381> Merci d\'envoyer un lien valide!');
                                            ...
                                            if(!embed.title && !embed.description && !embed.image?.url) return message.channel.send('<:false:881803557274255381> Votre embed doit contenir au moins soit un titre, soit une description, soit une image.');

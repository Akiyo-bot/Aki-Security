...
exports.run = (client, message, args) => {
    ...
    collector.on('collect', message => {
        if(nb_caract === "nd" && nombres === "nd" && symboles === "nd"){
            ...
            if(parseInt(message.content) > 100) return message.reply('Min : 0 | Max : 100')
    ...
    collector.on('end', (collected, reason) => {
        ...
        if(reason === "ok") {
            ...
            return message.channel.send('<a:verif:722725514912989254> Mot de passe envoyé en message privé !');
        }}
    ...
    console.log(times+"s","[INFO]","\x1b[0m","Command: "+"password"+" executed | By: "+message.author.displayName+" | In server: "+message.guild.name);
}

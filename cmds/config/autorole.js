...
module.exports.run = async (client, message, args) => {
    ...
    if(!auto) return message.channel.send(`⚠️ Le plugins d'autorole n'est pas activé, faites \`${prefix}enable autorole\` pour l'activer!`);

    const memberMsg = await message.channel.send('Envoyez le rôle à donner à tous les **nouveaux membres**.\nEnvoyez \'aucun\' pour ne pas en donner.\nEnvoyez \'skip\' pour annuler étape');
    ...
    memberCollector.on('collect', async (collectedMember) => {
        ...
        let role = null;
        if(collectedMember.content.toLowerCase() === 'skip') {
            return message.channel.send('⚠️ Commande annuler')
        } else {
        if(collectedMember.content.toLowerCase() !== 'aucun') {
            ...
            if(role.id == roleauto) return message.channel.send('⚠️ Ce rôle est le même que celui actuellement défini.');
        }
        ...
        if(collectedMember.content.toLowerCase() === 'skip') {
            ...
            if(collectedBot.content.toLowerCase() !== 'aucun') {
                ...
                if(botRole.id == roleauto) return message.channel.send('⚠️ Ce rôle est le même que celui actuellement défini.');
            }
            ...
            db.delete(`${message.guild.id}.config.autorole.role`)
            db.set(`${message.guild.id}.config.autorole.role`, role.id)
            db.set(`${message.guild.id}.config.autorole.rolebot`, botRole.id)
            return message.channel.send('✅ L\'autorole a bien été configuré.');
        }
        });
    }
